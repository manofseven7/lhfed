...
class BaseFedarated(object):
    def __init__(self, params, learner, dataset):
        ...
        self.statistic = defaultdict(dict)
    ...
    def test_resulting_model(self):
        ...
        #print('\n***ct(tot_correct): ', tot_correct, ', ns(num_samples):', num_samples)
    ...
    def statistics(self, r_num, ts_acc=None, tr_acc=None, valid_acc=None, ts_acc_per_client=None,
                   tr_acc_per_client=None, valid_acc_per_client=None):
        self.statistic[int(r_num)]['accs'] = [ts_acc, tr_acc, valid_acc]
        self.statistic[int(r_num)]['client_acc'] = [ts_acc_per_client.tolist(),
                                                    tr_acc_per_client.tolist(),
                                                    valid_acc_per_client.tolist()]

    def do_statistics(self, time):
        run_time = "%.2f seconds" % time
        print(run_time)
        sum_ts_acc = 0
        sum_tr_acc = 0
        sum_valid_acc = 0
        ...
        for s in self.statistic:
            if not all(v == 0 for v in self.statistic[s]['accs']):
                sum_ts_acc += self.statistic[s]['accs'][0]
                sum_tr_acc += self.statistic[s]['accs'][1]
                sum_valid_acc += self.statistic[s]['accs'][2]
            else:
                flag = True
        avg_ts_acc = sum_ts_acc / self.num_rounds
        avg_tr_acc = sum_tr_acc / self.num_rounds
        avg_valid_acc = sum_valid_acc / self.num_rounds
        avs_str = "Average Test Accuracy: {:10.3f}, Average Train Accuracy: {:10.3f}, Average Validation Accuracy: {:10.3f}".format(
            avg_ts_acc, avg_tr_acc, avg_valid_acc)
        print(avs_str)
        plot_valid_acc_data = []
        plot_ts_acc_data = []
        plot_tr_acc_data = []
        for s in self.statistic:
            if not all(v == 0 for v in self.statistic[s]['accs']):
                plot_ts_acc_data.append(self.statistic[s]['accs'][0])
                plot_tr_acc_data.append(self.statistic[s]['accs'][1])
                plot_valid_acc_data.append(self.statistic[s]['accs'][2])
        f = plt.figure(figsize=[11, 8])
        plt.plot(range(0, self.num_rounds if flag else self.num_rounds + 1), [v for v in plot_ts_acc_data])
        plt.plot(range(0, self.num_rounds if flag else self.num_rounds + 1), [v for v in plot_tr_acc_data])
        plt.plot(range(0, self.num_rounds if flag else self.num_rounds + 1), [v for v in plot_valid_acc_data])
        plt.xlabel('Rounds\n\n' + avs_str + '\n' + run_time)
        plt.ylabel('Accuracy')
        plt.legend(["Test Accuracy", "Train Accuracy", "Validation Accuracy"])
        plt.title(self.dataset + "-" + self.optimizer + "-" + str(self.data_partition_seed) + "-" +
                  str(self.q) + "-" + str(self.sampling))
        # plt.gca().xaxis.set_major_locator(mticker.MultipleLocator(1))
        plt.gca().xaxis.set_major_locator(mticker.MaxNLocator(integer=True))
        f.savefig("./res/" + self.dataset + "-" + self.optimizer + "-" + str(self.data_partition_seed) + "-" +
                  str(self.q) + "-" + str(self.sampling) + ".pdf")
        with open("./res/" + self.dataset + "-" + self.optimizer + "-" + str(self.data_partition_seed) + "-" +
                          str(self.q) + "-" + str(self.sampling) + ".json", "w") as outfile:
            json.dump(self.statistic, outfile, sort_keys=True)
