...
max2 = max(test_accuracies2)
max3 = max(test_accuracies2)
# max4 = max(test_accuracies2)
final_max = max(max0, max1, max2, max3)#, max4)
test_accuracies0[:] = [final_max - e for e in test_accuracies0]
test_accuracies1[:] = [final_max - e for e in test_accuracies1]
test_accuracies2[:] = [final_max - e for e in test_accuracies2]
test_accuracies3[:] = [final_max - e for e in test_accuracies3]
...
# plt.plot(np.asarray(rounds4), np.asarray(test_accuracies4), linewidth=1.0, label=r'q-FedAvg, q=2', color="#76b5c5")
plt.plot(np.asarray(rounds1)[::sampling_rate[0]], np.asarray(test_accuracies1)[::sampling_rate[0]], '--', linewidth=3.0, label=r'lhfed, n=2')
...
f.savefig("convergence-speed__lhfed-qffedavg-afl__vehicle.pdf")
# f.savefig("convergence-speed__hgfed-qffedavg-afl__synthetic.pdf")
